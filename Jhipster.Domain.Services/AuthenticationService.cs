...
namespace Jhipster.Domain.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        private async Task<User> LoadUserByUsername(string username)
        {
            ...
            if (PhoneNumberAttribute.IsValid(username))
            {
                var userByPhone = await _userManager.Users.FirstOrDefaultAsync(u => u.PhoneNumber.Equals(username));
                if (userByPhone != null)
                    return userByPhone;
            }
            ...
            if (userByLogin == null)
                throw new UsernameNotFoundException($"Tài khoản hoặc mật khẩu không đúng, vui lòng thử lại");
            return userByLogin;
        }
